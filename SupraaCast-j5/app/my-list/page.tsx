      ...
      <>
        ...
        <main className="flex min-h-screen flex-col items-center justify-center px-4">
          <div className="max-w-md text-center">
            <Heart className="mx-auto mb-4 h-16 w-16 text-amber-500/40" />
  ...
  return (
    <>
      ...
      <main className="min-h-screen px-4 pt-24 pb-12 lg:px-8">
        ...
        {/* Tabs */}
        <div className="mt-6 flex gap-1 rounded-lg border border-amber-900/30 bg-card/50 p-1 w-fit">
          <button
        ...
        {items.length === 0 ? (
          <div className="mt-16 flex flex-col items-center text-center">
            {activeTab === "liked" ? (
              <Heart className="mb-4 h-12 w-12 text-amber-500/30" />
            ) : (
              <Eye className="mb-4 h-12 w-12 text-amber-500/30" />
            )}
            ...
            {items.map((item) => {
              ...
              return (
                <div
                  ...
                  className="group relative overflow-hidden rounded-lg border border-amber-900/20 bg-card/50 transition-all hover:border-amber-700/40 hover:shadow-lg hover:shadow-amber-900/20"
                >
                  <Link href={href}>
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/5 text-red-400 backdrop-blur-md opacity-0 transition-all hover:bg-red-500 hover:text-white group-hover:opacity-100"
                  ...
                  {/* Type badge */}
                  <span className="absolute top-2 left-2 rounded-md bg-amber-600/80 px-2 py-0.5 text-[10px] font-semibold text-white uppercase">
                    {item.type === "movie" ? "Film" : "Serie"}
                  </span>

                  <div className="p-3">
                    <Link href={href}>
                      <h3 className="truncate text-sm font-semibold text-foreground transition-colors hover:text-amber-400">
                        {item.title}
                      </h3>
                    </Link>
