...
export function TVDetailClient({ show, seasonDetails }: TVDetailClientProps) {
  ...
  function getStreamUrl(season: number, episode: number) {
    return getSeriesPlayerUrl(show.id, season, episode)
  }
      ...
      <section className="px-4 py-6 lg:px-8">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-foreground backdrop-blur-md transition-all hover:bg-white/10"
            >
            ...
            {seasonDropdownOpen && (
              <div className="absolute right-0 top-full z-20 mt-1 max-h-60 w-48 overflow-y-auto rounded-lg border border-amber-700/40 bg-card shadow-xl">
                {seasonDetails.map((season, index) => (
                  <button
                    ...
                    className={`w-full px-4 py-2.5 text-left text-sm transition-colors hover:bg-amber-950/50 ${
                      index === selectedSeason ? "bg-amber-900/30 text-amber-300" : "text-foreground"
                    }`}
                  >
                    {season.name}
                  </button>
        ...
        <div className="flex flex-col gap-3">
          {episodes.map((episode) => (
            <div
              key={episode.id}
              className="group flex flex-col gap-4 rounded-lg border border-amber-900/20 bg-card/50 p-4 transition-colors hover:border-amber-700/40 hover:bg-card sm:flex-row"
            >
                ...
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="text-sm font-semibold text-foreground">
                      <span className="text-amber-400">E{episode.episode_number}</span>{" "}
                      {episode.name}
                    </h3>
