    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {STREAMING_PROVIDERS.map((provider) => (
        <Link
          ...
          className="group relative flex flex-col items-center justify-center overflow-hidden rounded-2xl border border-amber-800/20 p-8 transition-all duration-300 hover:border-amber-600/40 hover:shadow-xl hover:shadow-amber-900/30 hover:-translate-y-1 min-h-[180px] sm:min-h-[200px]"
        >
          {/* Gradient background */}
          <div
            className={`absolute inset-0 bg-gradient-to-br ${PROVIDER_GRADIENTS[provider.slug] || "from-amber-900 to-amber-950"} opacity-90 transition-opacity duration-300 group-hover:opacity-100`}
          />
          {/* Subtle inner glow */}
          <div
