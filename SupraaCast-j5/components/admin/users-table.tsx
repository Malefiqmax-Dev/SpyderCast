...
export function UsersTable({ users, currentUserId, onRefresh }: UsersTableProps) {
  ...
  return (
    <div className="rounded-xl border border-amber-800/30 bg-card/50">
      {/* Header */}
      <div className="flex flex-col gap-3 border-b border-amber-800/20 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-lg border border-amber-800/40 bg-secondary pl-9 pr-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500/50 sm:w-64"
          />
        </div>
      </div>
          ...
          <tbody className="divide-y divide-amber-800/10">
            {paged.map((user) => {
              ...
              return (
                <tr
                  key={user.id}
                  className={`transition-colors hover:bg-amber-950/30 ${isProcessing ? "opacity-50" : ""}`}
                >
                  <td className="whitespace-nowrap px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-foreground">
                        ...
                        {isSelf && (
                          <span className="ml-2 rounded-full bg-amber-600/20 px-2 py-0.5 text-[10px] font-semibold text-amber-400">
                            Vous
                          </span>
                        )}
                      </span>
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-amber-800/20 px-5 py-3">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              disabled={page === 0}
              className="rounded-lg p-1.5 text-muted-foreground transition-all hover:bg-white/5 hover:text-foreground backdrop-blur-sm disabled:opacity-30"
